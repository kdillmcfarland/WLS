...
```{r Rural.urban2}
summary(aov(Met.g$shannon ~ RU57*RU11, data = vars.g))
summary(aov(Met.g$chao ~ RU57*RU11, data = vars.g))
```
...
gp.se.gs.boxplot = data.frame(
  ...
  group = c(rep("1.spouse", length(match.BC.gp.se)), rep("2.sibling", length(match.BC.gs)), rep("3.unrelated", length((KW.results.final[KW.results.final$V6 == "dist" & complete.cases(KW.results.final),]$V2)))))
